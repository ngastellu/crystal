...
class Atom:
    ...
    def getAOparams(self): #obtain Gaussian fit parameters for aspherical electron scattering of the atom's different
        ...
        with open('../data/ffs_sorted.dat','r') as fo:
            ...
            flines = fo.readlines()
            for line in flines:
                if line.split()[0]==self.name and line.split()[1]!='f(tot)' and line.split()[1]!='f(core)': #ignore total FF; we are only interested in AO-specific data
                    self.AOparams.append(map(float,line.split()[3:])) #creates an array of the form[[a params for core],[b params for core]
                                                                    #,...[a's for last AO],[b's for the last AO]]
    ...
    def getSFa(self): #calculates the aspherical structure factor for atom in question
        ...
        if dim == 1: #dealing with only s AO
            for k in range(8):
                self.SFa = np.add(self.AOparams[0][k]*np.exp(-self.AOparams[1][k]*np.square(s_grid)), self.SFa)
            ...
            for k in range(8):
                FF_s = np.add(self.AOparams[0][k]*np.exp(-1.0*self.AOparams[1][k]*np.square(s_grid)), FF_s)
                FF_p0 = np.add(self.AOparams[2][k]*np.exp(-1.0*self.AOparams[3][k]*np.square(s_grid)), FF_p0) 
                ff_p1 = np.add(self.AOparams[4][k]*np.exp(-1.0*self.AOparams[5][k]*np.square(s_grid)), ff_p1) 
            ...
            for k in range(8):
                FF_s = np.add(self.AOparams[0][k]*np.exp(-self.AOparams[1][k]*np.square(s_grid)), FF_s)
                ff_d1 = np.add(self.AOparams[2][k]*np.exp(-self.AOparams[3][k]*np.square(s_grid)), ff_d1) 
                ff_d2 = np.add(self.AOparams[4][k]*np.exp(-self.AOparams[5][k]*np.square(s_grid)), ff_d2)
                ff_d3 = np.add(self.AOparams[6][k]*np.exp(-self.AOparams[7][k]*np.square(s_grid)), ff_d3)
