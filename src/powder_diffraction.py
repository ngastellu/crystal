...
os.chdir('/'.join(input_file.split('/')[:-1])) #change to directory where input file is stored
system = Crystal(input_file)
...
for atm in system.Basis:
    ...
    if system.Asymm == True: #determines whether to use asymmetrical form factors or not
...
for h in [-i for i in range(-6,7)]:
    for k in [-i for i in range(-6,7)]:
        for l in [-i for i in range(-6,7)]:
            ...
            theta = np.arctan2(np.linalg.norm(np.cross(lattVs[2],kVec)), np.dot(lattVs[2],kVec))
            SFtmp = 0.0
            if kNorm <= k_grid[100] and kNorm!=0:
                ...
                for atm in system.Basis:
                    ...
                    for j in range(len(atm.XYZ)):
                        ...
                        if system.Asymm == True: #in this case, we take into account the angular dependence of atomic factors
                            position = lattVs[0]*atm.XYZ[j][0] + lattVs[1]*atm.XYZ[j][1] + lattVs[2]*atm.XYZ[j][2]
                            theta = np.arctan2(np.linalg.norm(np.cross(lattVs[2],position)), np.dot(lattVs[2],position))
                        ...
                        else: #in this case we assume spherical form factors
                            position = lattVs[0]*atm.XYZ[j][0] + lattVs[1]*atm.XYZ[j][1] + lattVs[2]*atm.XYZ[j][2]
